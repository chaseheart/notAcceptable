...
public class ServicePerformanceService {
	...
	public Map<String, Object> findservicePerformance(User user, Date dateStart, Date dateEnd) {
		...
		List<SPAppCountDto> spList = servicePerformanceRepository.findAppTimesFromApplication(dateStart, dateEnd, user.getId(), SysStaticConst.STATE);

		Map<String, SPAppCountDto> sPAppCountDtoMap = new HashMap<>();
		for (SPAppCountDto s : spList) {
			sPAppCountDtoMap.put(s.getDay(), s);
		}
		for (ServicePerformanceDto dto : dtoList) {
			...
			if (!ObjectUtils.isEmpty(sPAppCountDtoMap.get(dto.getDay()))) {
				dto.setVacationApp(sPAppCountDtoMap.get(dto.getDay()).getVacationApp());
				dto.setUnusualAttendanceApp(sPAppCountDtoMap.get(dto.getDay()).getUnusualAttendanceApp());
				dto.setWorkOvertimeApp(sPAppCountDtoMap.get(dto.getDay()).getWorkOvertimeApp());
			}
	...
	public List<ServicePerformanceDto> findServicePerformanceList(User user, Date dateStart, Date dateEnd) {
		return servicePerformanceRepository.findServicePerformanceByUserAndDeleteFlag(user, dateStart, dateEnd, SysStaticConst.NOTDELE);
	}
...
//		map.put("claimingExpenses", ceList);
	...
	public Map<String, Object> getServicePerformaneAndVacationForm(Long ruFlowId) {
		...
		map.put("history", hiFlowRepository.getByRuFlowId(ruFlowId));
		return map;
	}
	...
	public Integer insertServicePerformanceByExcel(List<Object> result, User insertUser) {
		int failure = 0;
		for (int i = 1; i < result.size(); i++) {
			...
			// 。考勤开始时间
			String startTime = ObjectUtils.isEmpty(list.get(9)) ? "" : list.get(9).toString();
			// 。考勤结束时间
			String endTime = ObjectUtils.isEmpty(list.get(10)) ? "" : list.get(10).toString();
			// 。用户
			...
			if (sp != null) {
				...
				if (restDay.indexOf(day) == -1) {
					if (finalStart != null && finalEnd != null) {
						servicePerformance.setAbsenceTime(9 - Dateutil.hourCompareTo(finalStart, finalEnd));
					} else {
						servicePerformance.setAbsenceTime(8);
					}
				}
