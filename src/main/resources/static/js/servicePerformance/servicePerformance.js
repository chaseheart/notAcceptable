		...
		resultTable : [ {
			...
			render : function(h, params) {
				...
				var content = h("div", {
					...
					attrs : {
						href : '/outBusiness/index?id=' + params.row.id + '&date=' + params.row.date,
					}
				}, '外出公务申请') ])
				functionList.push(show);
				...
				for (var i = 0; i < monthDay; i++) {
					...
					if (!$.isEmptyObject(servicePerformance)) {
						...
						if (servicePerformance[keyDay] != undefined) {
							...
							let oaStart = !$.isEmptyObject(servicePerformance[keyDay].oaStart) ? servicePerformance[keyDay].oaStart.substring(0, servicePerformance[keyDay].oaStart.length - 3) : '';
							let oaEnd = !$.isEmptyObject(servicePerformance[keyDay].oaEnd) ? servicePerformance[keyDay].oaEnd.substring(0, servicePerformance[keyDay].oaEnd.length - 3) : '';
							let clockIn = '';
							if (oaStart != '' || oaEnd != '') {
								clockIn = oaStart + ' - ' + oaEnd;
							}
							...
							// if
							// (this.getDateWeek(servicePerformance[keyDay].day)
							// == '六' ||
							// this.getDateWeek(servicePerformance[keyDay].day)
							// == '日') {
							// result = '加';
							...
							resultData[i] = {
								...
								startTime : !$.isEmptyObject(oaFinalStart) ? oaFinalStart.substring(0, oaFinalStart.length - 3) : '',
								endTime : !$.isEmptyObject(oaFinalEnd) ? oaFinalEnd.substring(0, oaFinalEnd.length - 3) : '',
								overtime : '',
								status : '',
								absenteeism : servicePerformance[keyDay].absence,
								clockIn : clockIn,
								result : result,
								unusualAttendance : servicePerformance[keyDay].unusualAttendanceApp == '0' ? '' : servicePerformance[keyDay].unusualAttendanceApp,
								vacation : servicePerformance[keyDay].vacationApp == '0' ? '' : servicePerformance[keyDay].vacationApp,
								overtime : servicePerformance[keyDay].workOvertimeApp == '0' ? '' : servicePerformance[keyDay].workOvertimeApp
							}
						} else {
							resultData[i] = {
								date : keyDay,
