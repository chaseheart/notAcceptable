...
public interface ClaimingExpensesRepository extends JpaRepository<ClaimingExpenses, Long> {
	 ...
	 * @return
	 */
	@Query(" SELECT a FROM ClaimingExpenses a WHERE a.user.id = :user AND a.deleteFlag = :deleteFlag AND a.ruFlow.state != 6")
	public List<ClaimingExpenses> findByUserAndDeleteFlag(Long user, Boolean deleteFlag);
	 ...
	 * @return
	 */
	@Query(" SELECT a FROM ClaimingExpenses a WHERE a.user.id = :user AND a.deleteFlag = :deleteFlag AND (a.ruFlow.state = 6 OR a.ruFlow.state = 0)")
	public List<ClaimingExpenses> findByUserAndDeleteFlagEnd(Long user, Boolean deleteFlag);
	...
	@Query("select new com.isolver.dto.PendingDto(CONCAT(a.ruFlow.id,'' )as appId, "
			...
			+ " from ClaimingExpenses a where a.ruFlow.dealPeople= :dealPeople and a.ruFlow.deleteFlag = :deleteFlag and a.user.username like :username and a.user.workId like :workId and a.ruFlow.state > 1 and a.ruFlow.state < 6")
	public List<PendingDto> findApplicationDtoByUndo(User dealPeople, Boolean deleteFlag, String workId,
			String username);
	...
	public List<PendingDto> findMyApplicationDto(User user);
	...
	@Query("select new com.isolver.dto.PendingDto(CONCAT(a.ruFlow.id,'' )as appId, "
			+ " CONCAT(a.user.username,'' ) as appUsername, CONCAT(a.ruFlow.insertTime,'' ) as appStartTime, CONCAT(a.ruFlow.dealPeopleNow.username,'' ) as dealPeople, "
			+ " CONCAT(a.ruFlow.updateTime,'' ) as appEndtTime )"
			+ " from ClaimingExpenses a where a.ruFlow.dealPeople.id= :dealPeople and a.ruFlow.deleteFlag = :deleteFlag ")
	public List<PendingDto> findApplicationDtoByUndoWechat(Long dealPeople, Boolean deleteFlag);
	...
	public ClaimingExpenses findDataByRuFlowIdAndDeleteFlag(Long id, Boolean deleteFlag);
}
