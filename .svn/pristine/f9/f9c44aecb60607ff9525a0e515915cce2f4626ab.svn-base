...
public interface WorkOvertimeRepository extends JpaRepository<WorkOvertime, Long> {
	...
	@Query("select new com.isolver.dto.ApplicationDto(CONCAT(a.ruFlow.id,'' )as appId, " + " CONCAT(a.workOvertimeStart,'' ) as appStart, CONCAT(a.workOvertimeEnd,'' ) as appEnd,"
			+ " CONCAT(a.ruFlow.dealPeople.username,'' ) as assigner,  CONCAT(a.workContent,'' ) as content , CONCAT(a.ruFlow.state,'' ) as state )"
	...
	@Query("select new com.isolver.dto.PendingDto(CONCAT(a.ruFlow.id,'' )as appId, "
			...
			+ " from WorkOvertime a where a.ruFlow.dealPeople= :dealPeople and a.ruFlow.deleteFlag = :deleteFlag and a.user.username like :username and a.user.workId like :workId and a.ruFlow.state > 1 and a.ruFlow.state < 4")
	...
	@Query("select new com.isolver.dto.wechat.PendingDtoWechat(CONCAT(a.ruFlow.id,'' ) as appId,  CONCAT(a.ruFlow.updateTime,'' ) as appEndTime, '加班申请' as appName, (case when a.ruFlow.state = 1 then '再申请' when a.ruFlow.state = 2  or a.ruFlow.state = 3 then '审批中' end) as appstate, CONCAT(a.ruFlow.insertTime,'' ) as appStartTime) from WorkOvertime a where a.user = :user and a.ruFlow.deleteFlag = :deleteFlag and a.ruFlow.state < :state and a.ruFlow.state > 0"  )
	 ...
	 * @return
	 */
	@Query(" SELECT a FROM WorkOvertime a WHERE a.user = :user AND a.deleteFlag = :deleteFlag AND (a.ruFlow.state = 4 OR a.ruFlow.state = 0)")
	public List<WorkOvertime> findByUserAndDeleteFlagEnd(User user, Boolean deleteFlag);
