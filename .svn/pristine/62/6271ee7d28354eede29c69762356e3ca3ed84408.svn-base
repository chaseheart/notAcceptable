...
public class HolidayController {
	...
	public ModelAndView indexView2(String ruFlowId) {
		...
		if (!"undefined".equals(ruFlowId) && null != ruFlowId) {
			...
			Map<String, Object> map = servicePerformanceService.getServicePerformaneAndVacationForm(ids);
			modelAndView.addObject("vacation", JsonUtil.getJsonArray(map.get("vacationDto")));
	...
	public Result<Object> Application(@ModelAttribute HolidayForm holidayForm,
		...
		try {
			...
			if(model.equals(HtmlConst.APPLICATIONAGAIN)) {
				holidayService.restartFlow(holidayForm, user, Long.valueOf(ruFlowId));
			}else {
