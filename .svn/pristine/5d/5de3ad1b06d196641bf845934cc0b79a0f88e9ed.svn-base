...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.changeUpDown=function(){
        	if(jQuery("#arrow").hasClass("glyphicon-chevron-down")){
        		jQuery("#arrow").removeClass("glyphicon-chevron-down");
        		jQuery("#arrow").addClass("glyphicon-chevron-up");
        	}else{
        		jQuery("#arrow").addClass("glyphicon-chevron-down");
        		jQuery("#arrow").removeClass("glyphicon-chevron-up");
        	}
        }
            ...
            $http({method: 'GET', url: KISBPM.URL.getStencilSet()}).success(function (data, status, headers, config) {
                ...
                for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) 
                {
                    ...
                    var stencilItem = {'id': data.stencils[stencilIndex].id,
                        ...
                        'hide': data.stencils[stencilIndex].hide};
                    if(stencilItem.hide){
                    	continue;
                    }
