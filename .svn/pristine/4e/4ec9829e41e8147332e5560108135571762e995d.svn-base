...
public interface VacationRepository extends JpaRepository<Vacation, Long> {
	...
	@Query("select new com.isolver.dto.PendingDto(CONCAT(a.ruFlow.id,'' )as appId, "
			...
			+ " from Vacation a where a.ruFlow.dealPeople= :dealPeople and a.ruFlow.deleteFlag = :deleteFlag and a.user.username like :username and a.user.workId like :workId and a.ruFlow.state > 1 and a.ruFlow.state < 4")
	 ...
	 * @return
	 */
	@Query(" SELECT a FROM Vacation a WHERE a.user = :user AND a.deleteFlag = :deleteFlag AND (a.ruFlow.state = 4 OR a.ruFlow.state = 0)")
	public List<Vacation> findByUserAndDeleteFlagEnd(User user, Boolean deleteFlag);
